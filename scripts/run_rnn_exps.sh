...
do
   ...
   CUBLAS_WORKSPACE_CONFIG=:4096:8 python ./src/adding_memory_rnn.py --problem adding --init quantum --pickle-stats --seed $i --storage ./numbers/storage-2-manhattan-unshuffled-32bit-080321.pkl --storage-pos $((5120*$i))
done
...
do
   echo "Quantum-init experiment no: $i "
   CUBLAS_WORKSPACE_CONFIG=:4096:8 python ./src/adding_memory_rnn.py --problem memory --init quantum --pickle-stats --seed $i --storage ./numbers/storage-2-manhattan-unshuffled-32bit-080321.pkl --storage-pos $((5120*$i))
done
